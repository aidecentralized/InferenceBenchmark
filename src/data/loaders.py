...
class DataLoader():
    ...
    def setup_data_pipeline(self):
        ...
        self.trainloader = torch.utils.data.DataLoader(
            ...
            shuffle=True, num_workers=5, drop_last=True) # in case running on CPU based machines -> make num_workers=0

        self.testloader = torch.utils.data.DataLoader(
            test_dataset, batch_size=self.config["test_batch_size"],
            shuffle=False, num_workers=5, drop_last=True) # in case running on CPU based machines -> make num_workers=0
